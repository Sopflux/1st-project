...
public class RegisterFrame extends JFrame {
	...
	public RegisterFrame(LoginFrame lf) {
		...
		JLabel jl_announce = new JLabel("                                             *아이디와 닉네임 중복확인을 해주세요");
		Font f = new Font("SansSerif", Font.BOLD,11);
		jl_announce.setFont(f);
			...
			public void actionPerformed(ActionEvent e) {
				...
				int re = dao.insertAccount(a);
					if(re==1) { 
						...
						int choice = JOptionPane.showConfirmDialog(null, "로그인페이지로 이동하시겠습니까?", "종료", JOptionPane.YES_NO_OPTION, JOptionPane.INFORMATION_MESSAGE);
						if(choice==0) {
								lf.setVisible(true);
								rf.setVisible(false);
						}
