...
public class MainDAO {
	...
	// 추천 영화 (top 5) 출력
	public ArrayList<HashMap<String,Object>> Recommendation(String a_nick){
		...
		String sql = "select distinct m.m_no, m.m_image, m.kr_name\r\n"
				...
				+ "where m.m_no = l.m_no and a.a_no = l.a_no and a.a_nick = ?)\r\n"
				+ "and a_no != (select a_no from account where a_nick = ?)\r\n"
				+ "group by a_no order by like_count desc) \r\n"
				+ "where rownum <= 3) and l.a_no not in (\r\n"
				+ "select a.a_no from movie m, likes l, account a\r\n"
				+ "where m.m_no = l.m_no and a.a_no = l.a_no and a.a_nick = ?) and rownum <= 5\r\n"
				+ "union\r\n"
		...
		try {
			...
			pstmt.setString(1, a_nick);
			pstmt.setString(2, a_nick);
			pstmt.setString(3, a_nick);
